...
#ways, name = get_ways(*coords[0])
#roads = get_road_sections(intersection_list=ways, road_name=name)
#roads_dict = conversion_list_dict(roads)
#dict_distances = distance_from_segment(coords[0],roads_dict)

#result = find_optimal(dict_distances)
#listeinter=roads_dict[result]
#section=[result[0],result[1],listeinter[0],listeinter[1]]
...
for troncon in result:
    ...
    if len(coords)==1:
        test={coords[0]:1}
    listeinter=roads_dict2[coords[0]]
    listeinterinter=listeinter[troncon]
    section=[troncon[0],troncon[1],listeinterinter[0],listeinterinter[1]]
    test=get_order_in_segment(coords,section)
